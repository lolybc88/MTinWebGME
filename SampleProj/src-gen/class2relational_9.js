    ...
    modeltransformation.prototype.main = function (callback) {
        ...
        self.extractDataModel()
            .then(function () {
                ...
                if (self.isMetaTypeOf(elem, self.META['Attribute']) === true) {
                 	var a = elem;
                                     	
                                     	var nav0 = a;
                                     	
                                     	var nav0 = self.core.getParent(nav0);
                                     	var nav0 = self.core.getAttribute(nav0, 'isAbstract');
                                     	var left = nav0;
                                     	var right = false;
                                     	var filter = left	==
                                     ...
                                     var trace0 = {
                	        				...
                	        				col = self.core.createNode({
                                            parent: trgModelNode,
                                            base: self.META.Column
                                        });
                                        self.core.setAttribute(col, 'name', 'Column');
                                        //--
                		                        ...
                		                        var left = nav0;
                		                        var exp = left;
                		                        self.core.setAttribute(col, 'nname', exp);
                		                        //--
                                        self.core.setRegistry(col, 'position', {x: 50, y: 100});
                                        self.trgElements[self.core.getPath(col)] = col;
                                        trace0.srcElementIds.push(key);
                                        trace0.trgElementId = self.core.getPath(col);
                                        trace0.ruleName = 'Attribute2Column';
                                        trace0.position = 1;
                                        self.traces[trace0.srcElementIds + '-' + trace0.position] = trace0;
                        				;
                                     	}
                ...
                if (self.isMetaTypeOf(elem, self.META['Class']) === true) {
                 	var c = elem;
                                     	...
                                     	/* Elements creation */
                		                        ...
                		                        var nav0 = self.getChildren(nav0);
                		                        var left = nav0;
                		                        var right = null;
                		                        var condition = left	
                		                        !=
                		                        ...
                		                        var exp = left;
                		                        self.core.setPointer(t, 'key', exp);
                		                        //--
                		                        ...
                		                        var nav0 = self.getChildren(nav0);
                		                        var collect = [];
                		                        for (var ia = 0; ia < nav0.length; ia += 1) {
                		                        	var a = nav0[ia];
                		                        	var left = a;
                		                        	var left = left;
                		                        					var left = self.resolve(left, 1);
